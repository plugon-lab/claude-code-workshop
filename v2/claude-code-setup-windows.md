...
### STEP 1-2. VS Code 실행 확인

바탕화면 또는 시작 메뉴에서 **Visual Studio Code**를 실행합니다.

{% hint style="success" %}
✅ **성공** — VS Code 첫 화면이 뜨면 설치 완료입니다.
{% endhint %}

{% hint style="warning" %}
⚠️ **"Windows에서 PC를 보호했습니다" 경고창이 나오면?**  
...
{% hint style="success" %}
✅ **성공** — 화면 아래쪽에 입력창이 열리고 상단에 **powershell** 이라고 표시되면 완료입니다.
{% endhint %}

{% hint style="warning" %}
⚠️ **"bash" 또는 다른 텍스트가 보이면?**  
...
아래 명령어를 복사해서 터미널에 붙여넣고 **엔터**를 누르세요.
...
{% hint style="success" %}
✅ **성공** — "Successfully installed" 메시지가 나타나면 완료입니다.
{% endhint %}

{% hint style="warning" %}
⚠️ **"winget을 찾을 수 없습니다" 에러가 나면?**  
[nodejs.org](https://nodejs.org/ko) 에 접속 → **LTS 버전** 다운로드 → 설치 파일 실행  
설치 완료 후 STEP 2-1로 돌아와 터미널을 새로 열고 계속 진행하세요.
{% endhint %}
...
{% hint style="success" %}
✅ **성공** — "Successfully installed" 또는 "이미 설치되어 있습니다" 메시지 모두 정상입니다.
{% endhint %}
...
{% hint style="warning" %}
⚠️ 이 단계를 건너뛰면 다음 명령어가 작동하지 않을 수 있습니다.
{% endhint %}

---

## PART 3. Claude Code(CLI) 설치

---

### STEP 3-1. Claude Code 설치
...
{% hint style="warning" %}
⚠️ **"command not found" 또는 "찾을 수 없습니다" 에러가 나면?**  
터미널을 완전히 닫고 새로 여세요.  
...
## PART 4. Claude Code 실행 & 로그인

---

### STEP 4-1. Claude Code 실행

아래 명령어를 입력하고 엔터를 누르세요.
...
{% hint style="warning" %}
⚠️ **브라우저가 자동으로 열리지 않으면?**  
터미널에 표시된 URL을 복사해서 브라우저 주소창에 직접 붙여넣으세요.
{% endhint %}

---

### STEP 4-3. 첫 번째 대화 해보기
...
3. 패널에 대화창이 나타나면 연동 완료입니다

{% hint style="success" %}
✅ **성공** — 패널에 대화창이 나타나면 연동 완료입니다.
{% endhint %}
...
{% hint style="warning" %}
⚠️ **"로그인하지 않음 · /login을 실행하십시오" 메시지가 나오면?**  
`Ctrl+Shift+P` → `Developer: Reload Window` 로 창을 재시작하세요.
{% endhint %}

---

### STEP 5-4. 패널에서 첫 대화 해보기

패널 아래 입력창에 아래 문장을 입력하고 엔터를 눌러보세요.
...
| `스크립트를 실행할 수 없습니다` | 실행 정책 미적용 | STEP 2-4 실행 정책 변경 단계 재실행 |
...
*Plug:ON — AI를 실무에 연결하다 | Plug in. Power on.*
